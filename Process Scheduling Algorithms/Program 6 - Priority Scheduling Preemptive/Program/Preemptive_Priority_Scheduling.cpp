...
void Preemptive_Priority_Scheduling(Process P_Array[], int T_Process)
{
    ...
    while (!pque.empty() || processIterator < T_Process)
    {
        if (!pque.empty())
        {
            ...
            pque.pop();
            AddTimeToArray(processCpuAllocated, timeArray, timer, 1);
            timer += 1;
...
int main()
{
    ...
    cout << "Preemptive Priority Scheduling Process Scheduling Alogorithm\n";
    Print_Bars();
    int T_Process;
